...
class Bureaucracy:
    name: str
    purpose: str
    efficiency: float = 0.1  # Komiteler genelde %10 verimle çalışır
...
class DistractionSystem:
    def __init__(self):
        self.committees = []
        self.illusion_of_action = 0.0  # Halkın "bir şeyler yapılıyor" algısı

    def form_committee(self, crisis_type: str) -> str:
        """
        Gerçek sorunu çözmek yerine, o sorun hakkında konuşacak bir kurul oluşturur.
        Bu, kaynak harcar ama isyanı bastırır.
        """
        new_committee = Bureaucracy(
            name=f"The Royal Commission on {crisis_type}",
            purpose="Investigate and Monitor"
        )
        self.committees.append(new_committee)
        self.illusion_of_action += 20.0

        return f"ANNOUNCEMENT: A High-Level Committee has been formed to study '{crisis_type}'! The experts are now in charge."

    def run_bureaucracy(self, ship_resources):
        """Her tur çalışır, kaynak yer ama çok az iş yapar."""
        total_drain = 0
        logs = []

        for comm in self.committees:
            drain = 5  # Her komite kaynak tüketir (kahve, kağıt, toplantı)
            comm.budget_consumed += drain
            total_drain += drain
            comm.active_turns += 1

            # Komiteler zamanla hantallaşır
            if comm.active_turns > 5:
                comm.efficiency *= 0.5
                logs.append(f"REPORT: The '{comm.name}' is deadlocked in debate. No output produced.")
            else:
                logs.append(f"REPORT: The '{comm.name}' has released a preliminary PDF about the issue.")

        self.illusion_of_action *= 0.90  # Etkisi zamanla azalır
        return total_drain, logs
