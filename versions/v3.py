class SimulationAnalytics:
    ...
    def generate_report(self):
        ...
        self._analyze_psychological_trajectory()
        self._analyze_social_dynamics()
        self._analyze_power_shifts()
        self._analyze_moral_decay()
        self._analyze_key_moments()

    def _analyze_psychological_trajectory(self):
        """Analyze how characters changed psychologically"""
        print(f"\n🧠 PSYCHOLOGICAL TRAJECTORY:")

        radicalized = [c for c in self.ship.characters if c.is_radicalized]
        if radicalized:
            print(f"  ⚠️  {len(radicalized)} characters became radicalized:")
            for char in radicalized:
                print(f"    • {char.name} ({char.ideology.value})")

        traumatized = [c for c in self.ship.characters
                       if c.ptsd.calculate_total_impact(self.ship.day) > 50]
        if traumatized:
            print(f"  😰 {len(traumatized)} characters suffered severe trauma:")
            for char in traumatized:
                impact = char.ptsd.calculate_total_impact(self.ship.day)
                print(f"    • {char.name} (PTSD impact: {impact:.1f})")

        # Average stress progression
        if self.ship.characters:
            avg_stress = sum(c.stress for c in self.ship.characters if c.is_alive) / max(1, sum(
                1 for c in self.ship.characters if c.is_alive))
            print(f"  📈 Average stress level: {avg_stress:.1f}/100")

    def _analyze_social_dynamics(self):
        """Analyze alliance formation and breakdown"""
        print(f"\n👥 SOCIAL DYNAMICS:")

        total_alliances = len(self.ship.alliance_network.alliances)
        active_alliances = len(self.ship.alliance_network.get_active_alliances())
        broken_alliances = total_alliances - active_alliances

        print(f"  🤝 Alliances formed: {total_alliances}")
        print(f"  ✓ Still active: {active_alliances}")
        print(f"  ✗ Broken: {broken_alliances}")

        # Network density
        network = SocialNetwork(self.ship.characters)
        density = network.calculate_network_density()
        print(f"  🕸️  Social network density: {density:.1f}%")

        # Most influential
        influencers = network.identify_influencers(3)
        print(f"  ⭐ Most influential characters:")
        for char_id in influencers:
            char = next((c for c in self.ship.characters if c.id == char_id), None)
            if char:
                print(f"    • {char.name}")

    def _analyze_power_shifts(self):
        """Analyze how power changed hands"""
        print(f"\n⚡ POWER DYNAMICS:")

        print(f"  👑 Captain authority trajectory:")
        print(f"    Start: 100.0")
        print(f"    End: {self.ship.captain_authority:.1f}")
        print(f"    Change: {self.ship.captain_authority - 100:.1f}")

        print(f"  📊 Political events:")
        print(f"    Protests: {self.ship.protests_held}")
        print(f"    Concessions: {self.ship.concessions_granted}")
        print(f"    Mutinies: {self.ship.mutinies_attempted}")

        if self.ship.power_structure and self.ship.power_structure.parallel_structure_exists:
            print(f"  ⚠️  Shadow government emerged")

    def _analyze_moral_decay(self):
        """Analyze Overton window shifts"""
        print(f"\n⚖️  MORAL DECAY:")

        print(f"  📉 Overton Window Shifts:")
        normalized_behaviors = []
        for behavior, level in self.ship.overton_window.behaviors.items():
            if level in [AcceptabilityLevel.ACCEPTABLE, AcceptabilityLevel.SENSIBLE,
                         AcceptabilityLevel.POPULAR, AcceptabilityLevel.POLICY]:
                if behavior in ['violence', 'theft', 'scapegoating', 'mutiny']:
                    normalized_behaviors.append((behavior, level.value))

        if normalized_behaviors:
            print(f"  ⚠️  Previously unacceptable behaviors now normalized:")
            for behavior, level in normalized_behaviors:
                print(f"    • {behavior} → {level}")

        print(f"  💔 Collective guilt: {self.ship.ethical_system.collective_guilt:.1f}/100")

        # Cultural norms that collapsed
        collapsed_norms = [n for n in self.ship.cultural_evolution.norms if n.acceptance < 30]
        if collapsed_norms:
            print(f"  💀 Cultural norms that collapsed:")
            for norm in collapsed_norms:
                print(f"    • {norm.name}")

    def _analyze_key_moments(self):
        """Identify turning points"""
        print(f"\n🔑 KEY TURNING POINTS:")

        # Find most impactful events
        impactful_events = sorted(
            self.ship.event_generator.event_history,
            key=lambda e: e.severity,
            reverse=True
        )[:5]

        for i, event in enumerate(impactful_events, 1):
            print(f"  {i}. Day {event.day}: {event.description}")
            print(f"     (Impact: {event.severity:.1f})")

        # Foreshadowing payoffs
        if self.ship.narrative_engine.foreshadowing:
            revealed = [f for f in self.ship.narrative_engine.foreshadowing if f['revealed']]
            if revealed:
                print(f"\n  📖 Foreshadowing that paid off:")
                for item in revealed:
                    print(f"    • {item['hint']} → {item['payoff']}")


class AlternateScenario:
    """What-if scenario simulator"""

    @staticmethod
    def run_scenario(scenario_type: str):
        """Run alternate timeline"""
        print(f"\n{'=' * 70}")
        print(f"🔮 ALTERNATE SCENARIO: {scenario_type}")
        print(f"{'=' * 70}")

        if scenario_type == "early_mutiny":
            print("\nWhat if mutiny succeeded on Day 5?")
            print("  → Ship turns south immediately")
            print("  → 90% survival rate")
            print("  → Society becomes more democratic")
            print("  → But: Violence normalized, authority collapsed")

        elif scenario_type == "everyone_listens":
            print("\nWhat if everyone listened to the steward?")
            print("  → Ship turns south on Day 3")
            print("  → 100% survival rate")
            print("  → But: Would they learn the lesson?")
            print("  → Probably repeat same mistakes later")

        elif scenario_type == "perfect_captain":
            print("\nWhat if captain turned south voluntarily?")
            print("  → Ship saved, authority maintained")
            print("  → 95% survival rate")
            print("  → Society remains hierarchical")
            print("  → But: This scenario is extremely unlikely")

        elif scenario_type == "total_chaos":
            print("\nWhat if early civil war broke out?")
            print("  → Multiple factions fighting")
            print("  → Ship forgotten in chaos")
            print("  → Sinks even faster")
            print("  → 10% survival rate")

        elif scenario_type == "plague":
            print("\nWhat if disease outbreak occurred Day 1?")
            print("  → Half die from disease before ship sinks")
            print("  → Survivors too weak to mutiny")
            print("  → Ship drifts north unmanned")
            print("  → 5% survival rate")


class InteractiveMode:
    """Allow user to control one character"""

    def __init__(self, ship, player_character):
        self.ship = ship
        self.player = player_character
        self.actions_taken = []

    def get_player_action(self) -> str:
        """Get player's action for the day"""
        print(f"\n🎮 YOU ARE: {self.player.name}")
        print(f"   Your goal: Save the ship by turning it south")
        print(f"\n   Available actions:")
        print(f"   1. Speak out about danger")
        print(f"   2. Form alliance with someone")
        print(f"   3. Try to convince captain")
        print(f"   4. Organize protest")
        print(f"   5. Attempt mutiny")
        print(f"   6. Do nothing")

        choice = input("\n   Your choice (1-6): ").strip()

        if choice == '1':
            return self._speak_out()
        elif choice == '2':
            return self._form_alliance()
        elif choice == '3':
            return self._convince_captain()
        elif choice == '4':
            return self._organize_protest()
        elif choice == '5':
            return self._attempt_mutiny()
        else:
            return "nothing"

    def _speak_out(self) -> str:
        """Player warns about danger"""
        print(f"\n   You: 'We must turn south! The danger is real!'")

        # Effectiveness based on influence and speaking ability
        effectiveness = (self.player.influence + self.player.speaking_ability) / 200

        if random.random() < effectiveness:
            print(f"   Some people are listening...")
            self.player.influence += 5
            return "spoke_out_success"
        else:
            print(f"   Others: 'Stop being hysterical!'")
            self.player.influence -= 3
            return "spoke_out_failure"

    def _form_alliance(self) -> str:
        """Player forms alliance"""
        available = [c for c in self.ship.characters
                     if c.is_alive and c.id != self.player.id
                     and c.id not in self.player.alliances]

        if not available:
            print(f"   No one available to ally with")
            return "alliance_none"

        print(f"\n   Available characters:")
        for i, char in enumerate(available[:5], 1):
            print(f"   {i}. {char.name} ({char.ideology.value})")

        choice = input(f"   Choose (1-{min(5, len(available))}): ").strip()

        try:
            idx = int(choice) - 1
            if 0 <= idx < min(5, len(available)):
                target = available[idx]
                self.player.alliances.add(target.id)
                target.alliances.add(self.player.id)
                print(f"   ✓ Alliance formed with {target.name}")
                return f"alliance_{target.id}"
        except:
            pass

        return "alliance_failed"

    def _convince_captain(self) -> str:
        """Try to convince captain"""
        captain = next((c for c in self.ship.characters if c.role == 'captain'), None)
        if not captain or not captain.is_alive:
            print(f"   Captain is not available")
            return "captain_unavailable"

        print(f"\n   You attempt to reason with the captain...")

        # Very low chance of success
        if random.random() < 0.05:
            print(f"   🎉 MIRACLE! Captain agrees to turn south!")
            self.ship.heading = 180
            return "convinced_captain"
        else:
            print(f"   Captain: 'I know what I'm doing. Return to your station.'")
            self.ship.captain_authority += 2  # You reinforced his authority
            return "captain_refused"

    def _organize_protest(self) -> str:
        """Organize protest"""
        print(f"\n   You organize a protest about the ship's heading...")

        supporters = [c for c in self.ship.characters
                      if c.is_alive and c.stress > 60]

        if len(supporters) < 3:
            print(f"   Not enough support for protest")
            return "protest_failed"

        print(f"   {len(supporters)} people join your protest!")
        self.ship._organize_protest()
        return "protest_organized"

    def _attempt_mutiny(self) -> str:
        """Attempt mutiny"""
        print(f"\n   ⚠️  You attempt MUTINY!")

        allies = [c for c in self.ship.characters
                  if c.id in self.player.alliances and c.is_alive]

        if len(allies) < 4:
            print(f"   ❌ Not enough allies! You need at least 4.")
            print(f"   You are punished for attempted mutiny!")
            self.player.stress += 30
            self.player.influence -= 20
            return "mutiny_insufficient"

        self.ship._attempt_mutiny()
        return "mutiny_attempted"
...
class SaveSystem:
    ...
    @staticmethod
    def save_game(ship, filename="ship_save.json"):
        ...
        save_data = {
            ...
            'captain_authority': ship.captain_authority,
            'characters_alive': [c.id for c in ship.characters if c.is_alive],
            'protests': ship.protests_held,
            'mutinies': ship.mutinies_attempted
        }
    ...
    @staticmethod
    def load_game(filename="ship_save.json"):
        """Load saved state"""
        import json
...
def main():
    """Enhanced main with multiple modes"""
    print("=" * 70)
    print("🚢 SHIP OF FOOLS - ADVANCED SOCIAL SIMULATION")
    print("=" * 70)
    print("\nBased on Theodore Kaczynski's allegory")
    print("\n📖 SELECT MODE:")
    print("  1. Standard Simulation (watch it unfold)")
    print("  2. Interactive Mode (control one character)")
    print("  3. Fast-forward Mode (skip to end)")
    print("  4. Alternate Scenarios (what-if analysis)")
    ...
    if mode == '4':
        # Alternate scenarios
        print("\n🔮 ALTERNATE SCENARIOS:")
        print("  1. Early successful mutiny")
        print("  2. Everyone listens to steward")
        print("  3. Perfect captain turns south")
        print("  4. Total chaos - civil war")
        print("  5. Early plague outbreak")

        scenario_choice = input("\nScenario (1-5): ").strip()
        scenarios = ["early_mutiny", "everyone_listens", "perfect_captain",
                     "total_chaos", "plague"]

        if scenario_choice.isdigit() and 1 <= int(scenario_choice) <= 5:
            AlternateScenario.run_scenario(scenarios[int(scenario_choice) - 1])
    ...
    if mode == '2':
        ...
        for i, char in enumerate(ship.characters, 1):
            print(f"  {i}. {char.name} ({char.role}) - {char.ideology.value}")

        char_choice = input(f"\nChoice (1-{len(ship.characters)}): ").strip()
        try:
            idx = int(char_choice) - 1
            if 0 <= idx < len(ship.characters):
                player_char = ship.characters[idx]
                interactive = InteractiveMode(ship, player_char)
                print(f"\n✓ You are now {player_char.name}")
        except:
            print("\n⚠️  Invalid choice, using standard mode")
    ...
    try:
        while ship.simulation_active:
            ...
            if interactive:
                action = interactive.get_player_action()
                interactive.actions_taken.append((ship.day, action))
            ...
            # Auto-advance in fast-forward
            if fast_forward:
                if ship.day % 10 == 0:
                    print(f"  ⏩ Day {ship.day}...")
                continue
            ...
            # User input
            response = input("\n[ENTER=continue | s=status | save=save | q=quit]: ").strip().lower()
    ...
    # Generate analytics
    analytics = SimulationAnalytics(ship)
    analytics.generate_report()

    # Interactive mode summary
    if interactive:
        print(f"\n🎮 YOUR ACTIONS:")
        for day, action in interactive.actions_taken:
            print(f"  Day {day}: {action}")

        if ship.heading > 160:
            print(f"\n🎉 SUCCESS! Your actions helped save the ship!")
        else:
            print(f"\n💔 FAILURE. Despite your efforts, the ship sank.")
